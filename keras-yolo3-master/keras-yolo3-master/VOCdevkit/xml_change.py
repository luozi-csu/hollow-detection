...
def change_xml():
    ...
    """
    path = "C:/Users/Laona--LJJ/Documents/Tencent Files/496181303/FileRecv/datasets/trainSets/xml/"  # xml文件存放路径
    save_path = "VOC2007/Annotations/"  # 修改后的xml文件存放路径
    imgpath = "C:/Users/Laona--LJJ/PycharmProjects/pythonProject7/VOC2007/JPEGImages/"  # 新的照片path路径
    files = os.listdir(path)  # 读取路径下所有文件名
    for xmlFile in files:
        if xmlFile.endswith('.xml'):
            tree = ET.ElementTree(file=path + xmlFile)  # 打开xml文件，送到tree解析
            root = tree.getroot()  # 得到文档元素对象
            root[0].text = 'JPEGImages'   # root[0].text是annotation下第一个子节点中内容，直接赋值替换文本内容
            root[1].text = xmlFile
            root[1].text = root[1].text.replace('xml','png')  #修改根节点下的内容
